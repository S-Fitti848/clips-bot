...
Tres layouts:
  ...
  fullcam   la cara ocupa buena parte del frame (Just Chatting) → crop 9:16 centrado en la cara
  sincam    no hay cara estable → crop central 9:16 con zoom leve
...
@dataclass(frozen=True)
class Layout:
    tipo: str  # split | fullcam | sincam
    principal: Caja  # juego (split) o el único recorte (fullcam/sincam)
    camara: Caja | None
...
def decidir_layout(W: int, H: int, frames: list[list[Deteccion]], cam: Camara, render: Render) -> Layout:
    ...
    cara, presencia = cara_estable(W, H, frames)

    if cara is None or presencia < cam.min_presencia:
        alto = H / render.zoom_sin_camara
        return Layout("sincam", recorte(W, H, ratio_total, W / 2, H / 2, alto), None, presencia, cara)

    fx, fy, fw, fh = cara
    cx, cy = fx + fw / 2, fy + fh / 2

    if fw / W >= cam.cara_grande:
        return Layout("fullcam", recorte(W, H, ratio_total, cx, H / 2, H), None, presencia, cara)
